...
def order(request):
    ...
    if request.method == 'POST':
        ...
        order_d = Order(username=currentuser,user_email=u_email,image=prodimage,customer_contact=u_contact,product_name=prodname,product_id=prodid,product_quantity=packet,subtotal=price,delivery_charge=delivery_charge,total_amount=totalprice,delivery_address=u_delivery,ordernumber=ordernumber)  
        order_d.save()     
